...
| `workspace-connection-adapter.mjs` | Builds one workspace Connection Broker adapter per saved SSH instance; the main process registers and detaches adapters at runtime through the in-process server's `registerWorkspaceConnectionAdapter`/`unregisterWorkspaceConnectionAdapter` handle methods (startup alignment + `desktop_ssh_instances_set` diffs), so freshly created SSH instances appear as catalog connections without a restart. Each adapter resolves the current SSH tunnel status and private desktop-host client token on every operation, then forwards HTTP/SSE/WS only to ssh-manager-produced tunnel URLs; tunnel lifecycle stays with `ssh-manager`; renderers never see tunnel URLs or SSH material. |
...
Desktop settings updates share one serialized read-modify-write queue across the main process and SSH manager. Writes are atomic, settings containing credentials are restricted to the current OS user, and malformed settings fail closed instead of being replaced with an empty root.
