...
describe('fetchControlPlaneCatalogWorkspaces', () => {
  const snapshotResponse = (workspaces: unknown[], status = 200): Response => new Response(
    JSON.stringify({
      schemaVersion: 1,
      revision: 3,
      connections: [],
  ...
  test('maps a reachable catalog snapshot to descriptor shapes with auth headers', async () => {
    ...
    const result = await fetchControlPlaneCatalogWorkspaces({
    ...
    assert.equal(calls[0].url, 'http://control.test:3000/api/workspaces');
  ...
  test('preserves a path-prefixed control-plane origin', async () => {
    const { fetchImpl, calls } = stubFetch(() => snapshotResponse([]));
    await fetchControlPlaneCatalogWorkspaces({ origin: 'http://host:8080/openchamber/', fetchImpl });
    assert.equal(calls[0]?.url, 'http://host:8080/openchamber/api/workspaces');
  });

  test('returns null without fetching when no control plane origin is configured', async () => {
    const { fetchImpl, calls } = stubFetch(() => snapshotResponse([]));
    const result = await fetchControlPlaneCatalogWorkspaces({ origin: null, fetchImpl });
  ...
  test('returns null on a non-2xx catalog response', async () => {
    ...
    const result = await fetchControlPlaneCatalogWorkspaces({ origin: 'http://control.test', fetchImpl });
    assert.equal(result, null);
    assert.equal(calls.length, 1);
  });
  ...
  test('unreachable real fetch seam still answers capability_unavailable without fabricating an id', async () => {
    ...
    const response = await handleWorkspaceBridgeMessage(
      ...
      {
        readWorkspaceFolders: readFolders([FOLDER_ALPHA]),
        fetchCatalogWorkspaces: () => fetchControlPlaneCatalogWorkspaces({ origin: null, fetchImpl }),
      },
    );
    const data = response?.data as Record<string, unknown>;
    assert.equal(data.status, 'capability_unavailable');
