...
import { useActiveProjectCapabilities } from '@/projects/useActiveProject';
...
export type MobileProjectTab = 'changes' | 'files' | 'terminal' | 'notes' | 'mcp';

/** Quick MCP enable/disable toggles as a project pane, with its own slim
    action row (add server → settings, refresh) replacing the old fullscreen
    surface's header actions. */
const McpProjectPane: React.FC<{ onOpenMcpSettings: () => void }> = ({ onOpenMcpSettings }) => {
  const { t } = useI18n();
...
/** The project surfaces as tabs (Changes / Files / Terminal / Notes / MCP).
...
export const MobileProjectDrawer: React.FC<{
  open: boolean;
  onClose: () => void;
  tab: MobileProjectTab;
  onTabChange: (tab: MobileProjectTab) => void;
  ...
  // Terminal capability of the ACTIVE project's connection. When the
  ...
  // project / catalog not loaded) leaves the tab as-is — the legacy
  // ambient runtime keeps its current behavior until a project connection
  // is authoritative.
  ...
  const [visitedTabs, setVisitedTabs] = React.useState<ReadonlySet<MobileProjectTab>>(() => new Set());
  React.useEffect(() => {
    if (!open) return;
  ...
  const tabItems: SortableTabsStripItem[] = [
    ...
    // Hidden when the active project's connection cannot host a terminal.
    ...(terminalUnavailable
      ? []
        ...
        <div className="flex h-9 min-w-0 flex-1 items-center">
          ...
          {visible ? (
            <SortableTabsStrip
              items={tabItems}
              activeId={tab}
              onSelect={(id) => onTabChange(id as MobileProjectTab)}
              layoutMode="fit"
              variant="active-pill"
      ...
      <div className="min-h-0 flex-1 overflow-hidden">
        ...
        {visitedTabs.has('mcp') ? (
          <div className={cn('h-full', tab !== 'mcp' && 'hidden')}>
            <ErrorBoundary>
              <McpProjectPane onOpenMcpSettings={onOpenMcpSettings} />
            </ErrorBoundary>
          </div>
        ) : null}
      </div>
