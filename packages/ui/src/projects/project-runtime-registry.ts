import { createOpencodeServiceForSdk, createProjectOpencodeClient } from '@/lib/opencode/client';
...
import { projectScopeKey } from './identity';
import { rewriteRuntimePathToProject, projectRuntimePrefix, projectSdkBaseUrl } from './project-runtime-fetch';
import type { ProjectDescriptor, ProjectId } from './types';
import type {
...
export interface ProjectRuntimeHandle {
  ...
  scopeKey: string;
  directory: string;
  sdk: ReturnType<typeof createProjectOpencodeClient>;
  /**
  ...
  /** Project-bound RuntimeAPIs: OpenChamber-owned capabilities (files, git,
   * terminal, …) route through the project runtime proxy prefix on the
   * CURRENT control plane with the project directory header. Terminal
   * streaming owns a project-scoped socket and URL-auth token. */
  apis: RuntimeAPIs;
...
export interface ProjectRuntimeRegistry {
  get(project: ProjectDescriptor): ProjectRuntimeHandle;
  invalidate(projectId: ProjectId): void;
  dispose(): void;
}

/**
 * A project-scoped capability whose server contract does not exist yet.
 ...
 * ambient runtime.
 */
class ProjectCapabilityUnavailableError extends Error {
  readonly code = 'capability_unavailable';
  readonly status = 501;

  constructor(capability: string) {
    super(`${capability} is not available for a project runtime`);
 ...
 *
 * Builds one project-bound handle per project: an SDK client whose base
 * URL is the control-plane project prefix (`/api/projects/:id/runtime/api`)
 ...
 * the server resolves the project to its connection adapter.
 *
...
const createProjectUrlResolver = (projectId: ProjectId): RuntimeUrlResolver => {
  const prefix = projectSdkBaseUrl(projectId);
  return {
...
// registry builds a minimal project-bound implementation here: every
// OpenChamber-owned request goes to the project runtime proxy prefix
// (`/api/projects/:id/runtime/...`) on the CURRENT control plane with the
// project directory header. Terminal streaming uses the same project
// prefix for its WebSocket upgrade and never falls back to the ambient socket.
// ---------------------------------------------------------------------------

const PROJECT_DIRECTORY_HEADER = 'x-opencode-directory';

type ProjectApiFetch = (
  restPath: string,
  init?: RequestInit,
  directoryOverride?: string,
) => Promise<Response>;

const createProjectApiFetch = (
  projectId: ProjectId,
  directory: string,
  controlPlaneFetch: typeof fetch,
): ProjectApiFetch => {
  return (restPath, init = {}, directoryOverride) => {
    const rewritten = rewriteRuntimePathToProject(projectId, restPath);
    if (!rewritten) {
      return Promise.reject(new Error(`Not a runtime API path: ${restPath}`));
    }
    const headers = new Headers(init.headers);
    const targetDirectory = directoryOverride ?? directory;
    if (targetDirectory && !headers.has(PROJECT_DIRECTORY_HEADER)) {
      headers.set(PROJECT_DIRECTORY_HEADER, targetDirectory);
    }
...
const createProjectFilesApi = (apiFetch: ProjectApiFetch, directory: string): FilesAPI => ({
  ...
  async statFile(path: string, options): Promise<{ path: string; isFile: boolean; size: number; mtimeMs?: number }> {
    ...
    if (options?.allowOutsideProject) params.set('allowOutsideProject', 'true');
...
const PROJECT_URL_AUTH_SKEW_MS = 10_000;

type ProjectTerminalApiBundle = {
  api: TerminalAPI;
  dispose: () => void;
};

const createProjectTerminalSocketUrl = (projectId: ProjectId, token: string): string => {
  const path = `${projectRuntimePrefix(projectId)}/api/terminal/ws`;
...
const createProjectTerminalTransport = (
  projectId: ProjectId,
  controlPlaneFetch: typeof fetch,
  ...
  const refreshAuth = async (): Promise<string> => {
    if (token && expiresAt > Date.now() + PROJECT_URL_AUTH_SKEW_MS) return token;
    ...
    const request = (async () => {
      ...
      if (!response.ok) {
        clearToken();
        throw new Error(`Failed to mint project terminal URL auth token (${response.status})`);
      }
      ...
      if (!nextToken || !Number.isFinite(nextExpiresAt)) {
        clearToken();
        throw new Error('Project terminal URL auth token response was invalid');
      }
      token = nextToken;
  ...
  return new TerminalTransport({
    refreshAuth,
    openSocket: () => openSocket(createProjectTerminalSocketUrl(projectId, token)),
    clearUrlAuthToken: clearToken,
  });
};

const createProjectTerminalApi = (
  projectId: ProjectId,
  apiFetch: ProjectApiFetch,
  controlPlaneFetch: typeof fetch,
  openSocket: (url: string, protocols?: string[]) => RelayTunnelWebSocket,
): ProjectTerminalApiBundle => {
  const transport = createProjectTerminalTransport(projectId, controlPlaneFetch, openSocket);
...
const createProjectGitApi = (apiFetch: ProjectApiFetch, directory: string): GitAPI => {
...
const createProjectRuntimeApis = (
  projectId: ProjectId,
  directory: string,
  controlPlaneFetch: typeof fetch,
  openSocket: (url: string, protocols?: string[]) => RelayTunnelWebSocket,
): { apis: RuntimeAPIs; dispose: () => void } => {
  const apiFetch = createProjectApiFetch(projectId, directory, controlPlaneFetch);
  const terminal = createProjectTerminalApi(projectId, apiFetch, controlPlaneFetch, openSocket);
  return {
    apis: {
      runtime: { platform: 'web', isDesktop: false, isVSCode: false, label: `project:${projectId}` },
      files: createProjectFilesApi(apiFetch, directory),
      git: createProjectGitApi(apiFetch, directory),
      terminal: terminal.api,
      settings: {
        load: () => Promise.reject(new ProjectCapabilityUnavailableError('Project settings')),
        save: () => Promise.reject(new ProjectCapabilityUnavailableError('Project settings')),
      },
      permissions: {
        requestDirectoryAccess: () => Promise.resolve({ success: false, error: 'Directory access is managed by the project connection' }),
...
export const createProjectRuntimeRegistry = (dependencies: {
  maxRetained?: number;
  disposeGraceMs?: number;
  /** Pinned fetch seam for focused project transport tests. */
  controlPlaneFetch?: typeof fetch;
  /** Socket-open seam for terminal transport tests; defaults to the runtime
   * socket (relay tunnel or browser WebSocket). */
  openSocket?: (url: string, protocols?: string[]) => RelayTunnelWebSocket;
  /** SDK factory seam for tests; defaults to the project-bound SDK
  ...
  const createSdkClient = dependencies.createSdkClient ?? ((config: { baseUrl: string; directory: string; fetch?: typeof fetch }) => (
    createProjectOpencodeClient(config as { baseUrl: string; directory: string })
  ));

  const handles = new Map<ProjectId, ProjectRuntimeHandle>();
  const leaseCounts = new Map<ProjectId, number>();
  const idleTimers = new Map<ProjectId, ReturnType<typeof setTimeout>>();
  let evictionScheduled = false;
  // One pinned control-plane fetch shared by every project SDK client: the
  // project prefix is resolved against the CURRENT control plane, never the
  // Active Runtime.
  const controlPlaneFetch = dependencies.controlPlaneFetch ?? createControlPlaneFetch();

  const clearIdleTimer = (projectId: ProjectId): void => {
  ...
  const scheduleEviction = (): void => {
    ...
    setTimeout(() => {
      ...
      for (const [projectId, handle] of handles) {
        if ((leaseCounts.get(projectId) ?? 0) === 0 && handles.size > maxRetained) {
  ...
  const get = (project: ProjectDescriptor): ProjectRuntimeHandle => {
    const existing = handles.get(project.id);
    if (existing) {
      clearIdleTimer(project.id);
      return existing;
    }
    const sdk = createSdkClient({
      baseUrl: projectSdkBaseUrl(project.id),
      directory: project.canonicalPath,
      fetch: controlPlaneFetch,
    }) as ProjectRuntimeHandle['sdk'];
    const service = createOpencodeServiceForSdk({
      client: sdk,
      baseUrl: projectSdkBaseUrl(project.id),
      directory: project.canonicalPath,
      scopeKey: projectScopeKey(project.id),
      fetch: controlPlaneFetch,
      createScopedClient: (directory) => createProjectOpencodeClient({
        baseUrl: projectSdkBaseUrl(project.id),
        directory,
        fetch: controlPlaneFetch,
      }),
    });
    const projectApis = createProjectRuntimeApis(project.id, project.canonicalPath, controlPlaneFetch, openSocket);
    const handle: ProjectRuntimeHandle = {
      ...
      sdk,
      service,
      apis: projectApis.apis,
      urls: createProjectUrlResolver(project.id),
      retain: () => {
        leaseCounts.set(project.id, (leaseCounts.get(project.id) ?? 0) + 1);
        clearIdleTimer(project.id);
        let released = false;
        return () => {
          if (released) return;
          released = true;
          const next = Math.max(0, (leaseCounts.get(project.id) ?? 1) - 1);
          leaseCounts.set(project.id, next);
          if (next === 0) {
            const timer = setTimeout(() => {
              idleTimers.delete(project.id);
              if ((leaseCounts.get(project.id) ?? 0) === 0) {
                handles.delete(project.id);
                handle.dispose();
              }
            }, disposeGraceMs);
            idleTimers.set(project.id, timer);
          }
        };
      },
      dispose: () => {
        clearIdleTimer(project.id);
        leaseCounts.delete(project.id);
        handles.delete(project.id);
        projectApis.dispose();
      },
    };
  ...
  const invalidate = (projectId: ProjectId): void => {
  ...
  const dispose = (): void => {
    for (const handle of handles.values()) {
      clearIdleTimer(handle.projectId);
      handle.dispose();
    }
    handles.clear();
    leaseCounts.clear();
