...
import { projectSessionKey } from './identity';
import { fetchProjectCapabilities } from './catalog-client';
import { fetchProjectSessionSnapshot } from './session-index-client';
import {
  CatalogClientError,
 ...
 *   `projectCatalogV1` from the control plane on a SEPARATE channel. A
...
interface SessionIndexState {
  ...
  applyEvent: (event: ProjectSessionEvent) => void;
...
const isSessionSummary = (value: unknown): value is ProjectSessionSummary => {
  if (!value || typeof value !== 'object') return false;
  const summary = value as Partial<ProjectSessionSummary>;
...
const indexSessions = (sessions: ProjectSessionSummary[]) => {
...
export interface SessionIndexStoreDependencies {
  ...
  fetchSnapshot?: typeof fetchProjectSessionSnapshot;
}
...
export const createSessionIndexStore = (dependencies: SessionIndexStoreDependencies = {}) => {
  const fetchSnapshot = dependencies.fetchSnapshot ?? fetchProjectSessionSnapshot;
  ...
  applyEvent: (event) => set((state) => {
    ...
    if (event.type === 'session.removed') {
      ...
      const key = projectSessionKey(event.projectId, event.sessionId);
  ...
  refreshCapabilities: async () => {
    ...
    try {
      const capabilities = await fetchProjectCapabilities();
      ...
      // Only an authoritative `{ projectCatalogV1: false }` disables the
      // unified sidebar; a transient/unknown read keeps current behavior.
      set((state) => ({
        capabilitiesStatus: 'error',
        capabilitiesError: error instanceof CatalogClientError ? error.message : error instanceof Error ? error.message : 'Failed to load project capabilities',
...
export const useProjectSessionIndexStore = createSessionIndexStore();

export const selectSessionsForProject = (
  snapshot: ProjectSessionSnapshot | null,
  projectId: ProjectId,
): ProjectSessionSummary[] => (
  snapshot ? snapshot.sessions.filter((session) => session.projectId === projectId) : []
);

/**
 * Resolves the ACTIVE project from the current session selection.
 *
 * The session index is authoritative for project-bound sessions: only
 * sessions mapped to a project appear there (unassigned sessions live in a
 * separate diagnostics bucket and never surface under a project). When the
 * currently selected session is found in the index, its project is the
...
export const resolveActiveProjectId = (
  sessions: ProjectSessionSummary[] | undefined,
  ...
  return (byDirectory ?? matches[0]).projectId;
};
