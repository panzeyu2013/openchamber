...
import { createSessionIndexStore, selectSessionsForProject } from './session-index-store';
import {
  CatalogClientError,
...
let fetchSnapshotImpl: () => Promise<ProjectSessionSnapshot>;
...
const makeSession = (projectId: string, sessionId: string, overrides: Partial<ProjectSessionSummary> = {}): ProjectSessionSummary => ({
  key: `${projectId}\0${sessionId}`,
  projectId,
  connectionId: 'conn-1',
  upstreamSessionId: sessionId,
  directory: `/home/${projectId}`,
...
const makeEvent = (
  revision: number,
  type: ProjectSessionEvent['type'],
  payload: unknown,
  extra: Partial<ProjectSessionEvent> = {},
): ProjectSessionEvent => ({
  revision,
  connectionId: 'conn-1',
  projectId: 'ws-1',
 ...
 * is detectable. */
const makeCountingArrayProxy = (sessions: ProjectSessionSummary[], counters: Record<string, number>): ProjectSessionSummary[] => (
  new Proxy(sessions, {
  ...
  }) as ProjectSessionSummary[]
);

describe('project session index store', () => {
  beforeEach(() => {
  ...
  test('applyEvent session.removed removes exactly one session', async () => {
    ...
    store.getState().applyEvent(
      makeEvent(6, 'session.removed', {}, { projectId: 'ws-1', sessionId: 'ses-2' }),
    );
  ...
  describe('refresh/event reconciliation (ported from the retired global sessions store)', () => {
    test('a snapshot fetched before applied events never rolls them back', async () => {
      ...
      store.getState().applyEvent(makeEvent(12, 'session.removed', null, { projectId: 'ws-1', sessionId: 'ses-old' }));
  ...
  test('selectSessionsForProject filters by projectId and tolerates a null snapshot', () => {
    const snapshot = makeSnapshot(5, [
      makeSession('ws-1', 'ses-1'),
      makeSession('ws-2', 'ses-2'),
      makeSession('ws-1', 'ses-3'),
    ]);
    expect(selectSessionsForProject(snapshot, 'ws-1').map((session) => session.upstreamSessionId)).toEqual(['ses-1', 'ses-3']);
    expect(selectSessionsForProject(snapshot, 'ws-2').map((session) => session.upstreamSessionId)).toEqual(['ses-2']);
    expect(selectSessionsForProject(null, 'ws-1')).toEqual([]);
  });
  ...
  describe('reducer work is proportional to the affected entity (performance budget §17.5)', () => {
    ...
    const wrapCommittedSessionsWithCounter = (counters: Record<string, number>): ProjectSessionSummary[] => {
      const state = store.getState();
      const sessions = state.snapshot?.sessions ?? [];
      const proxied = makeCountingArrayProxy(sessions, counters);
      store.setState({
        snapshot: { ...(state.snapshot as ProjectSessionSnapshot), sessions: proxied },
      });
      return sessions;
    };
    ...
    test('upsert of one existing session never scans or rebuilds other entries', async () => {
      ...
      // and position (project A's event must not rebuild other entries).
    ...
    test('removal is membership-checked through the index and never filters the array', async () => {
      ...
      store.getState().applyEvent(
        makeEvent(6, 'session.removed', {}, { projectId: 'ws-1', sessionId: 'ses-2500' }),
      );
