...
const descriptor = (id: string, overrides: Partial<ProjectDescriptor> = {}): ProjectDescriptor => ({
  id,
  connectionId: 'conn-1',
  path: `/path/${id}`,
...
describe('project runtime registry', () => {
  let registry: ReturnType<typeof createProjectRuntimeRegistry> | undefined;
  ...
  test('get() creates a handle with project-scoped identity and sdk', () => {
    registry = createProjectRuntimeRegistry({ createSdkClient: fakeCreateSdkClient });
    const handle = registry.get(descriptor('ws-1', { canonicalPath: '/projects/alpha' }));
    expect(handle.scopeKey).toBe('project:ws-1');
    ...
    expect(sdkCalls).toEqual([{ baseUrl: '/api/projects/ws-1/runtime/api', directory: '/projects/alpha' }]);
  });
  ...
  test('project Git mutations carry the project directory query', async () => {
    const requests: Array<{ url: string; query?: unknown }> = [];
    registry = createProjectRuntimeRegistry({
    ...
    expect(requests).toEqual([{
      url: '/api/projects/ws-1/runtime/api/git/stage',
      query: { directory: '/projects/alpha' },
    }]);
  });

  test('project settings stay explicitly unavailable instead of using the ambient API', async () => {
    registry = createProjectRuntimeRegistry({ createSdkClient: fakeCreateSdkClient });
  ...
  test('a handle survives the grace window while retained, then is evicted after release', async () => {
    registry = createProjectRuntimeRegistry({ disposeGraceMs: 5, createSdkClient: fakeCreateSdkClient });
    const handle = registry.get(descriptor('ws-1'));
  ...
  test('re-acquiring a lease during the grace period cancels disposal', async () => {
    registry = createProjectRuntimeRegistry({ disposeGraceMs: 20, createSdkClient: fakeCreateSdkClient });
    const handle = registry.get(descriptor('ws-1'));
    handle.retain()();
    await sleep(10);
    expect(registry.get(descriptor('ws-1'))).toBe(handle);
  ...
  test('LRU eviction drops a released handle when over maxRetained', async () => {
    registry = createProjectRuntimeRegistry({ maxRetained: 2, createSdkClient: fakeCreateSdkClient });
    const first = registry.get(descriptor('ws-1'));
    first.retain()();
