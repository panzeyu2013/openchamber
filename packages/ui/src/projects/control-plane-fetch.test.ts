...
describe('control-plane-fetch pinning', () => {
  ...
  test('pins relative paths to the control-plane origin', async () => {
    ...
    await fetchPinned('/api/projects');
    expect(captured[0].url).toBe('https://cp.example/api/projects');
  });

  test('serializes project API query options onto the pinned URL', async () => {
    const fetchPinned = createControlPlaneFetch();
    await fetchPinned('/api/projects/proj-1/runtime/api/fs/list', {
      query: new URLSearchParams({ path: '/remote/project', respectGitignore: 'true' }),
    } as RequestInit);
    expect(captured[0].url).toBe(
      'https://cp.example/api/projects/proj-1/runtime/api/fs/list?path=%2Fremote%2Fproject&respectGitignore=true',
    );
  });
  ...
  test('rewrites window-origin absolute URLs to the control-plane base', async () => {
    const fetchPinned = createControlPlaneFetch();
    await fetchPinned(new Request('https://cp.example/api/projects/proj-1/runtime/api/session', { method: 'GET' }));
    expect(captured[0].url).toBe('https://cp.example/api/projects/proj-1/runtime/api/session');
  });

  test('preserves Request method, body, and headers while pinning its URL', async () => {
    const fetchPinned = createControlPlaneFetch();
    await fetchPinned(new Request('https://cp.example/api/projects/proj-1/runtime/api/session', {
      method: 'POST',
  ...
  test('prefers the injected local origin on desktop', async () => {
    ...
    const fetchPinned = createControlPlaneFetch();
    await fetchPinned('/api/projects');
    expect(captured[0].url).toBe('http://127.0.0.1:3901/api/projects');
  });
  ...
  test('keeps a deployment path prefix on the control plane', async () => {
    ...
    const fetchPinned = createControlPlaneFetch();
    await fetchPinned('/api/projects');
    expect(captured[0].url).toBe('https://host.example/chamber/api/projects');
  });
  ...
  test('an explicitly injected origin wins over every other source', async () => {
    ...
    const fetchPinned = createControlPlaneFetch();
    await fetchPinned('/api/projects');
    expect(captured[0].url).toBe('https://mobile-control-plane.example/api/projects');
  });
  ...
  test('non-http window origins answer control_plane_unavailable without dispatching', async () => {
    ...
    const fetchPinned = createControlPlaneFetch();
    const response = await fetchPinned('/api/projects');
