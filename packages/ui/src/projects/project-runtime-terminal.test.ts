...
// is process-global and leaks into other projects/sync test files).
...
const { createProjectRuntimeRegistry } = await import('./project-runtime-registry');
...
describe('project runtime terminal transport', () => {
  test('routes socket traffic and URL auth per project scope', async () => {
    sockets.length = 0;
    openedUrls.length = 0;
    let tokenRequests = 0;
    ...
    expect(openedUrls[0]).toContain('/api/projects/ws-1/runtime/api/terminal/ws');
    expect(openedUrls[1]).toContain('/api/projects/ws-2/runtime/api/terminal/ws');
  ...
  test('keeps messages, directories, terminal streams, permissions, and drafts isolated across 100 switches', async () => {
    ...
    try {
      ...
      const message = childStores.getChild('/same/repository', projectScopeKey('rapid-50'))?.getState().message['same-session']?.[0] as unknown as { info?: { id?: string } } | undefined;
      expect(message?.info?.id).toBe('message-50');
      const permission = childStores.getChild('/same/repository', projectScopeKey('rapid-50'))?.getState().permission['same-session']?.[0] as unknown as { id?: string } | undefined;
      expect(permission?.id).toBe('permission-50');
      expect(childStores.getChild('/same/repository', projectScopeKey('rapid-49'))?.getState().permission['same-session']?.[0]?.id).toBe('permission-49');
      const question = childStores.getChild('/same/repository', projectScopeKey('rapid-50'))?.getState().question['same-session']?.[0] as unknown as { id?: string } | undefined;
