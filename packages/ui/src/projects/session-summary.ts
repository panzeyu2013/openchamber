...
import type { ConnectionId, ProjectSessionSnapshot, ProjectSessionSummary } from './types';

/**
 * Session-index consumer surface (legacy full-session surfaces).
 *
 * The Session Index is the cross-project authority for session summaries.
 ...
 * - Live full-session data for the ACTIVE project continues to come from
 *   the project runtime handle's SDK (or the live child stores), and
 ...
 */
export const sessionFromSummary = (summary: ProjectSessionSummary): Session => ({
  id: summary.upstreamSessionId,
  slug: '',
  projectID: summary.projectId,
  workspaceID: summary.projectId,
