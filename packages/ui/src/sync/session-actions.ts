...
import { opencodeClient, ascendingId } from "@/lib/opencode/client"
...
let _getSdkForServer: ((serverId: string) => OpencodeClient) | null = null
...
export function setActionRefs(
  ...
  getDirectory: () => string,
  getSdkForServer?: (serverId: string) => OpencodeClient,
) {
  _sdk = sdk
  _childStores = childStores
  _getDirectory = getDirectory
  if (getSdkForServer) _getSdkForServer = getSdkForServer
}
...
function getServerIdForSession(sessionId: string): string | null {
  if (_childStores) {
    for (const { serverId, store } of _childStores.getAllEntries()) {
      if (store.getState().session.some((s) => s.id === sessionId)) {
        return serverId
      }
    }
  }
  ...
  return (session as Record<string, unknown>)?.serverId as string ?? null
}

function sdkForSession(sessionId?: string): OpencodeClient {
  const serverId = sessionId
    ? getServerIdForSession(sessionId)
    : useDirectoryStore.getState().currentServerId
  if (!serverId || serverId === "local") return sdk()
  if (_getSdkForServer) {
    const serverSdk = _getSdkForServer(serverId)
...
function dirStore() {
  ...
  const serverId = useDirectoryStore.getState().currentServerId ?? "local"
  return _childStores.getOrCreateChildStore(serverId, d)
}

function dirStoreForDirectory(directory: string, serverId?: string) {
  if (!_childStores) throw new Error("Child stores not initialized")
  if (!directory) throw new Error("No directory")
  return _childStores.getOrCreateChildStore(serverId ?? "local", directory)
}

type DirectoryStoreApi = ReturnType<ChildStoreManager["getOrCreateChildStore"]>

function getDirectoryStore(directory?: string, serverId?: string) {
  if (!_childStores) throw new Error("Child stores not initialized")
  const resolvedDirectory = directory || _getDirectory()
  if (!resolvedDirectory) throw new Error("No current directory")
  const effectiveServerId = serverId ?? useDirectoryStore.getState().currentServerId ?? "local"
  return _childStores.getOrCreateChildStore(effectiveServerId, resolvedDirectory)
}

export { getDirectoryStore }
...
function updateLiveSession(session: Session, directory?: string): void {
  ...
  if (directory) {
    const store = stores.findChildByDirectory(directory)
    if (!store) return
  ...
  for (const { store } of stores.getAllEntries()) {
    const current = store.getState().session
    const index = current.findIndex((item) => item.id === session.id)
    if (index === -1) continue
    const next = [...current]
...
function dirStoreForSession(sessionId: string): { store: DirectoryStoreApi; directory?: string } {
  ...
  const serverId = getServerIdForSession(sessionId) ?? "local"
  if (directory) {
    return { store: dirStoreForDirectory(directory, serverId), directory }
  }
  const d = dir()
  return { store: d ? dirStoreForDirectory(d, serverId) : dirStore(), directory: d }
}

function getSessionReplyClient(sessionId?: string): OpencodeClient {
  if (sessionId) {
    const serverSdk = sdkForSession(sessionId)
    if (serverSdk !== sdk()) return serverSdk
    const serverId = getServerIdForSession(sessionId)
    const directory = useSessionUIStore.getState().getDirectoryForSession(sessionId)
    if (directory) {
      return opencodeClient.getScopedSdkClient(directory, serverId ?? undefined)
    }
  }
  return sdk()
}
...
export async function createSession(
  ...
  try {
    ...
    const session = assertSdkData(result, "session.create")
...
function optimisticRemoveSession(sessionId: string, preferredDirectory?: string): SessionListSnapshot[] {
  ...
  const candidates: Array<{ directory: string; store: DirectoryStoreApi }> = []

  if (preferredDirectory) {
    for (const { directory, store } of _childStores.getAllEntries()) {
      if (directory === preferredDirectory) {
        candidates.push({ directory, store })
...
function restoreSessionListSnapshots(snapshots: SessionListSnapshot[]): void {
  ...
  for (const snapshot of snapshots) {
    const store = _childStores.getAllEntries().find((e) => e.directory === snapshot.directory)?.store
    if (!store) continue
...
export async function deleteSession(sessionId: string, _options?: Record<string, unknown>): Promise<boolean> {
  ...
  try {
    ...
    const deleted = await sdkForSession(sessionId).session.delete({ sessionID: sessionId, directory: sessionDirectory })
...
export async function deleteSessionInDirectory(sessionId: string, directory: string): Promise<boolean> {
  ...
  try {
    ...
    const result = await sdkForSession(sessionId).session.delete({ sessionID: sessionId, directory })
    if (result.data !== true) {
      throw new Error("session.delete failed: server did not confirm deletion")
    }
    useGlobalSessionsStore.getState().removeSessions([sessionId])
...
export async function archiveSession(sessionId: string): Promise<boolean> {
  ...
  try {
    await cleanupReviewMetadataBeforeDelete(sessionId, sessionDirectory)
    const result = await sdkForSession(sessionId).session.update({ sessionID: sessionId, directory: sessionDirectory, time: { archived: archivedAt } })
    if (result.data) {
...
export async function shareSession(sessionId: string): Promise<Session | null> {
  ...
  const session = result.data ? stripSessionDiffSnapshots(result.data) : null
  if (session) {
    useGlobalSessionsStore.getState().upsertSession(session)
...
export async function optimisticSend(input: {
  ...
  onOptimisticInsert?: (messageID: string) => void
  ...
  const sessionServerId = getServerIdForSession(input.sessionId) ?? "local"
  const store = targetDirectory ? dirStoreForDirectory(targetDirectory, sessionServerId) : dirStore()
  ...
  input.onOptimisticInsert?.(messageID)
...
export async function respondToPermission(
  ...
  const directory = resolveDirectoryForBlockingRequest("permission", sessionId, requestId)
    ...
    || dir()
  let client = getRequestReplyClient("permission", sessionId, requestId)
  if (serverId && serverId !== 'local' && _getSdkForServer) {
    const scoped = _getSdkForServer(serverId)
    if (scoped) client = scoped
  }
...
export async function revertToMessage(sessionId: string, messageId: string): Promise<void> {
  ...
  if (status && status.type !== "idle") {
    try {
      await sdkForSession(sessionId).session.abort({ sessionID: sessionId, directory })
    } catch {
      // ignore abort errors
    }
  }
  ...
  try {
    const result = await sdkForSession(sessionId).session.revert({ sessionID: sessionId, directory, messageID: messageId })
    const revertedSession = assertSdkData(result, "session.revert")
    const current = store.getState()
...
export async function refetchSessionMessages(sessionId: string): Promise<void> {
  const { store, directory } = dirStoreForSession(sessionId)
  const result = await sdkForSession(sessionId).session.messages({ sessionID: sessionId, directory, limit: MESSAGE_REFETCH_LIMIT })
...
export async function unrevertSession(sessionId: string): Promise<void> {
  ...
  const result = await sdkForSession(sessionId).session.unrevert({ sessionID: sessionId, directory })
...
export async function forkFromMessage(sessionId: string, messageId: string): Promise<void> {
  ...
  const result = await sdkForSession(sessionId).session.fork({ sessionID: sessionId, directory, messageID: messageId })
  const forkedSession = assertSdkData(result, "session.fork")
