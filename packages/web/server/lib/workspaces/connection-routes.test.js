...
const createApp = async (dependencies = {}) => {
  ...
  broker.registerAdapter(createLocalWorkspaceAdapter({ fs: fsPromises, path, normalizeDirectoryPath: (p) => p }));

  const onConnectionsChanged = dependencies.onConnectionsChanged ?? (() => {});
  registerWorkspaceCatalogRoutes(app, { catalogStore, connectionBroker: broker, profileStore, onConnectionsChanged, safeUpstreamValidator: testValidator });
...
describe('connection profile CRUD', () => {
  ...
  it('refuses to delete a connection still referenced by workspaces', async () => {
