...
(renderer side; the runtime proxy it needs is already in place and the
renderer now mounts the workspace runtime provider for local workspaces).
SyncProvider is still ambient-runtime bound for non-workspace selections;
`getOpencodeClient()`/`switchRuntimeEndpoint()` remain as migration facades
that new code must not call.
