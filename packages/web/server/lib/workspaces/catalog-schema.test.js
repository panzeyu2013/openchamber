...
const validWorkspace = (overrides = {}) => ({
  id: 'ws-1',
  connectionId: 'conn-1',
  path: '/tmp/project',
...
describe('validateCatalogDocument', () => {
  ...
  it('does not mutate the input document', () => {
    ...
    expect(input.workspaces[0].color).toBe('#ABC');
  });

  it('drops unknown private fields from connections and workspaces', () => {
    const input = validDocument({
      connections: [validConnection({ target: { kind: 'ssh', sshInstanceId: 'secret-ssh' }, credentialRef: 'secret-token' })],
      workspaces: [validWorkspace({ secret: 'data' })],
    });
    ...
    expect(result.workspaces[0]).not.toHaveProperty('secret');
  });
  ...
  it('returns null when connections or workspaces are not arrays', () => {
    ...
    expect(validateCatalogDocument(validDocument({ workspaces: 'nope' }))).toBeNull();
    expect(validateCatalogDocument(validDocument({ workspaces: undefined }))).toBeNull();
  });
  ...
  it('drops workspaces missing required fields', () => {
    const input = validDocument({
      workspaces: [
        validWorkspace({ id: '' }),
        validWorkspace({ id: 'ws-2', connectionId: '' }),
        validWorkspace({ id: 'ws-3', path: '' }),
        validWorkspace({ id: 'ws-4', canonicalPath: '' }),
        validWorkspace({ id: 'ws-5', label: '' }),
        validWorkspace(),
      ],
    });
    const result = validateCatalogDocument(input);
    expect(result.workspaces.map((entry) => entry.id)).toEqual(['ws-1']);
  });
  ...
  it('drops duplicate workspace ids', () => {
    const input = validDocument({
      workspaces: [validWorkspace(), validWorkspace({ canonicalPath: '/other' })],
    });
    const result = validateCatalogDocument(input);
  ...
  it('drops duplicate (connectionId, canonicalPath) locations', () => {
    const input = validDocument({
      workspaces: [validWorkspace(), validWorkspace({ id: 'ws-2' })],
    });
    const result = validateCatalogDocument(input);
    expect(result.workspaces).toHaveLength(1);
  });
...
describe('toWorkspaceDescriptor', () => {
  it('serializes the public workspace shape', () => {
    expect(toWorkspaceDescriptor({
      id: 'ws-1',
      connectionId: 'conn-1',
      path: '/tmp/project',
      canonicalPath: '/tmp/project',
  ...
  it('omits the color when it is invalid or missing', () => {
    const descriptor = toWorkspaceDescriptor({ id: 'ws-1', connectionId: 'c', path: '/p', canonicalPath: '/p', label: 'L' });
  ...
  it('returns null for a missing record', () => {
    expect(toWorkspaceDescriptor(null)).toBeNull();
  });
});
...
describe('validateCreateWorkspaceInput', () => {
  it('accepts a minimal valid input and normalizes it', () => {
    expect(validateCreateWorkspaceInput({ connectionId: ' local ', path: ' /tmp/project ' })).toEqual({
      connectionId: 'local',
      path: ' /tmp/project ',
      label: null,
      color: null,
  ...
  it('normalizes label trimming and hex color case', () => {
    expect(validateCreateWorkspaceInput({
      connectionId: 'local',
      path: '/tmp/project',
      label: '  My Project  ',
      color: '#ABC',
  ...
  it('rejects a missing connectionId with a typed error', () => {
    const failure = failureOf(captureError(() => validateCreateWorkspaceInput({ path: '/tmp/project' })));
  ...
  it('rejects a missing or empty path', () => {
    for (const inputPath of [undefined, '', '   ']) {
      const failure = failureOf(captureError(() => validateCreateWorkspaceInput({ connectionId: 'local', path: inputPath })));
      expect(failure).toBeInstanceOf(CatalogInputError);
  ...
  it('rejects non-object input', () => {
    for (const input of [null, 'x', 42]) {
      const failure = failureOf(captureError(() => validateCreateWorkspaceInput(input)));
      expect(failure).toBeInstanceOf(CatalogInputError);
      expect(failure.message).toBe('Workspace input must be an object');
    }
  });
});
...
describe('validateUpdateWorkspaceInput', () => {
  it('returns an empty patch for an empty input', () => {
    expect(validateUpdateWorkspaceInput({})).toEqual({});
  });

  it('trims labels', () => {
    expect(validateUpdateWorkspaceInput({ label: '  New Name  ' })).toEqual({ label: 'New Name' });
  });

  it('rejects an empty label', () => {
    for (const label of ['', '  ']) {
      const failure = failureOf(captureError(() => validateUpdateWorkspaceInput({ label })));
      expect(failure).toBeInstanceOf(CatalogInputError);
  ...
  it('normalizes hex colors and lowercases them', () => {
    expect(validateUpdateWorkspaceInput({ color: '#ABC' })).toEqual({ color: '#abc' });
    expect(validateUpdateWorkspaceInput({ color: '#AABBCC' })).toEqual({ color: '#aabbcc' });
  });

  it('removes the color when null is passed explicitly', () => {
    expect(validateUpdateWorkspaceInput({ color: null })).toEqual({ color: null });
    expect(validateUpdateWorkspaceInput({ color: undefined })).toEqual({ color: null });
  });

  it('rejects an invalid color', () => {
    for (const color of ['red', '#12', '']) {
      const failure = failureOf(captureError(() => validateUpdateWorkspaceInput({ color })));
      expect(failure).toBeInstanceOf(CatalogInputError);
  ...
  it('leaves the color untouched when absent', () => {
    expect(validateUpdateWorkspaceInput({ label: 'X' })).not.toHaveProperty('color');
  });

  it('keeps string orderKeys and rejects non-strings', () => {
    expect(validateUpdateWorkspaceInput({ orderKey: '42' })).toEqual({ orderKey: '42' });
    const failure = failureOf(captureError(() => validateUpdateWorkspaceInput({ orderKey: 42 })));
    expect(failure).toBeInstanceOf(CatalogInputError);
  ...
  it('rejects non-object input', () => {
    for (const input of [null, 'x']) {
      const failure = failureOf(captureError(() => validateUpdateWorkspaceInput(input)));
      expect(failure).toBeInstanceOf(CatalogInputError);
    }
  });
});
