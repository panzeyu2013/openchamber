 ...
 * - Only the documented workspace-capable `/api/...` path families are
 *   forwarded (the SDK base URL is `/api/workspaces/:workspaceId/runtime/api`,
 *   so every SDK path lands under `/api`). Control-plane namespaces such as
 *   `/api/workspaces`, `/api/connections`, `/api/client-auth`, `/api/system`
...
/**
 * Server capability flag `workspaceCatalogV1` (plan §20): an operator switch
 * that gates catalog/session-index mutations and the workspace runtime
 * proxy WITHOUT touching catalog data files (a pure read gate). The flag is
 * enabled unless `OPENCHAMBER_WORKSPACE_CATALOG_DISABLED=1` is set.
 *
 ...
 */
export const isWorkspaceCatalogDisabled = (env = process?.env ?? {}) => (
  typeof env.OPENCHAMBER_WORKSPACE_CATALOG_DISABLED === 'string'
  && env.OPENCHAMBER_WORKSPACE_CATALOG_DISABLED.trim() === '1'
);

export const resolveWorkspaceCatalogV1 = (dependencyValue) => (
  typeof dependencyValue === 'boolean' ? dependencyValue : !isWorkspaceCatalogDisabled()
);
 ...
 * workspace-owned Files/Git/Terminal APIs. A prefix entry matches only the
 ...
 * absent. */
const WORKSPACE_RUNTIME_API_PATH_PREFIXES = Object.freeze([
  '/api/agent',
  '/api/auth',
  '/api/command',
  '/api/event',
...
// This control-plane endpoint reports the machine home directory rather than
// reading inside the selected workspace. It must never cross a workspace
// runtime boundary, even though it shares the `/api/fs` family.
const NON_WORKSPACE_RUNTIME_API_PATHS = new Set(['/api/fs/home']);

// OpenCode's config endpoints are not a workspace API contract. Keep them out
// of the proxy and return a typed 501 instead of accidentally mutating the
// ambient/global runtime selected by the renderer.
const EXPLICITLY_UNAVAILABLE_WORKSPACE_RUNTIME_PATH_PREFIXES = Object.freeze([
  '/api/config',
]);

const MAX_WORKSPACE_RUNTIME_REQUEST_BYTES = 50 * 1024 * 1024;
const MAX_WORKSPACE_RUNTIME_RESPONSE_BYTES = 100 * 1024 * 1024;
...
const getRestPathname = (restPath) => {
  ...
  try {
    return new URL(restPath, 'http://workspace.local').pathname;
  } catch {
    return '';
  }
};

export const isForwardableWorkspaceRuntimePath = (restPath) => {
  const pathname = getRestPathname(restPath);
  if (pathname === '/api') return true;
  if (NON_WORKSPACE_RUNTIME_API_PATHS.has(pathname)) return false;
  if (EXPLICITLY_UNAVAILABLE_WORKSPACE_RUNTIME_PATH_PREFIXES.some((prefix) => (
    pathname === prefix || pathname.startsWith(`${prefix}/`)
  ))) return false;
  return WORKSPACE_RUNTIME_API_PATH_PREFIXES.some((prefix) => (
    pathname === prefix || pathname.startsWith(`${prefix}/`)
  ));
};

export const isWorkspaceRuntimeCapabilityUnavailablePath = (restPath) => {
  const pathname = getRestPathname(restPath);
  return EXPLICITLY_UNAVAILABLE_WORKSPACE_RUNTIME_PATH_PREFIXES.some((prefix) => (
    pathname === prefix || pathname.startsWith(`${prefix}/`)
  ));
};
...
export const stripWorkspaceUrlAuthToken = (restPath) => {
  if (typeof restPath !== 'string' || !restPath.includes('?')) return restPath;
  try {
    const url = new URL(restPath, 'http://workspace.local');
...
const resolveWorkspaceContext = async (workspaceId, { catalogStore, connectionBroker, credentialProvider = null }) => {
  ...
  return { workspace, profile: resolved.profile, adapter: resolved.adapter, credentialProvider };
};
...
export const registerWorkspaceRuntimeProxyRoutes = (app, dependencies) => {
  const {
    ...
    logger = null,
    maxRequestBytes = MAX_WORKSPACE_RUNTIME_REQUEST_BYTES,
    maxResponseBytes = MAX_WORKSPACE_RUNTIME_RESPONSE_BYTES,
  } = dependencies;
  ...
  const handleForward = (req, res) => {
    ...
    if (!parsed) {
      return sendJsonErrorCounted(res, 404, 'Unknown workspace runtime path', 'catalog_runtime_path_not_found');
    }
    if (!ALLOWED_METHODS.has(req.method)) {
      return sendJsonErrorCounted(res, 405, 'Method not allowed', 'catalog_runtime_method_not_allowed');
    }
    if (isWorkspaceRuntimeCapabilityUnavailablePath(parsed.restPath)) {
      return sendJsonErrorCounted(res, 501, 'This workspace capability is not available', 'capability_unavailable');
    }
    if (!isForwardableWorkspaceRuntimePath(parsed.restPath)) {
    ...
    if (isContentLengthOverLimit(req, maxRequestBytes) || requestBodyBytes > maxRequestBytes) {
      return sendJsonErrorCounted(res, 413, 'Workspace runtime request body is too large', 'catalog_runtime_body_too_large');
    }
    const forwardedRestPath = stripWorkspaceUrlAuthToken(parsed.restPath);
    ...
    return (async () => {
      ...
      try {
        ...
        logger?.log?.('[workspaces:proxy] forward failed', `workspace=${parsed.workspaceId} code=${error?.code ?? 'unknown'}`);
        if (!res.headersSent) {
...
const streamLimitError = () => {
  const error = new Error('Workspace runtime response body is too large');
...
const pipeUpstreamBody = async (body, res, signal = null, maxBytes = MAX_WORKSPACE_RUNTIME_RESPONSE_BYTES) => {
  if (!body) {
    res.end();
    return;
  }
...
const runWorkspaceUpgrade = async (parsed, req, socket, head, dependencies) => {
  const {
    ...
    logger = null,
    workspaceCatalogV1 = null,
  } = dependencies;
  try {
    // The flag-disabled state (§20) rejects every workspace runtime upgrade
    // with an explicit capability error — never a silent swallow and never
    // a tunnel to the upstream runtime.
    if (resolveWorkspaceCatalogV1(workspaceCatalogV1) === false) {
      rejectWebSocketUpgrade(socket, 501, 'The workspace catalog is disabled on this server');
      return;
    }
    ...
    try {
      ...
      logger?.log?.('[workspaces:proxy] workspace upgrade upstream failed', `workspace=${parsed.workspaceId} code=${error?.code ?? 'unknown'}`);
      rejectUpgradeError(socket, error, rejectWebSocketUpgrade);
      return;
    } finally {
      if (!upstream) release();
    ...
    logger?.log?.('[workspaces:proxy] workspace upgrade failed', `workspace=${parsed.workspaceId} code=${error?.code ?? 'unknown'}`);
