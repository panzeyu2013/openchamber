 ...
 *   workspace" flow can pick any folder on the current computer.
 * - `fetch`/`openEventStream`/`openWebSocket` are implemented in Phase 2
 *   (workspace runtime proxy); until then they return `capability_unavailable`.
 */
...
export const createLocalWorkspaceAdapter = (dependencies) => {
  ...
  const fetch = async (context, request, restPath) => {
    ...
    if (canonicalPath && !headers.has('x-opencode-directory') && !headers.has('x-openchamber-directory')) {
