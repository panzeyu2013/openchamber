/**
 * Workspace diagnostics route.
 *
 * `GET /api/workspaces/diagnostics` — desensitized control-plane snapshot
 * (plan §19 of docs/unified-workspace-session-architecture.md), served
 * behind the same base UI auth gate as every other workspace route:
 *
 ...
 * - migration completion/pending/failure state and the `workspaceCatalogV1`
 *   capability flag.
 *
...
export const registerWorkspaceDiagnosticsRoutes = (app, dependencies) => {
  const { getDiagnostics } = dependencies;

  app.get('/api/workspaces/diagnostics', async (_req, res) => {
    try {
      res.json(desensitizeDiagnostics(await getDiagnostics()));
    } catch (error) {
      sendError(res, 500, error instanceof Error ? error.message : 'Failed to read workspace diagnostics');
    }
  });
};
