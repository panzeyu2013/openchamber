 ...
 *
 * Maintains a LIGHTWEIGHT cross-connection index of project sessions:
 * - At most ONE upstream event stream per connection (never one per
 *   project); structural session events trigger debounced snapshot
 *   refreshes of that connection.
 * - Sessions are mapped to projects through the binding store; when no
 *   binding exists, an exact canonical-path match against the connection's
 *   projects is the only automatic fallback. Unmatched sessions go to the
 ...
 * - Stop generation: every stopObservingConnection increments a per-connection
 *   generation token; every async continuation (snapshot apply, debounced
 *   refresh, observer start/stream continuation) re-checks the token it
 *   captured before committing, so a stopped connection can never be
 *   resurrected by an in-flight refresh or a late stream resolution.
 ...
 * here — the active project's full sync loads those on demand.
 */
...
export const createSessionIndex = (dependencies) => {
  ...
  // Stop-generation tokens: each stopObservingConnection bumps the token, and
  // every in-flight async commit re-checks the token it captured before
  // touching state or emitting events. The tokens OUTLIVE the connection's
  // state entry (stop deletes it): re-observing a stopped connection starts a
  // fresh generation that still rejects commits from the old one.
  const stoppedGenerations = new Map();
  const eventListeners = new Set();

  const log = (message, detail) => {
    logger?.log?.(`[projects:session-index] ${message}`, detail ?? '');
  };

  const currentGeneration = (connectionId) => stoppedGenerations.get(connectionId) ?? 0;

  const isGenerationCurrent = (connectionId, generation) => currentGeneration(connectionId) === generation;
  ...
  const listProjectsForConnection = async (connectionId) => {
    const projects = await catalogStore.listProjectsForConnection(connectionId);
    return projects;
  };

  /** Exact-path fallback ONLY when no binding exists for the session. */
  const resolveProjectForSession = async (connectionId, session) => {
    const binding = await bindingStore.getBinding(connectionId, session.id);
    if (binding) return binding.projectId;
    const projects = await listProjectsForConnection(connectionId);
    const directory = normalizePath(session.directory ?? '');
    if (!directory) return null;
    const match = projects.find((project) => normalizePath(project.canonicalPath) === directory);
    return match?.id ?? null;
  };

  const applySnapshot = async (connectionId, payload, { partial = false, generation = 0 } = {}) => {
    // The connection may have been stopped while this snapshot was in flight:
    // never recreate its state or emit events for a dead connection.
    if (!isGenerationCurrent(connectionId, generation)) return false;
    ...
    for (const session of sessions) {
      ...
      const projectId = await resolveProjectForSession(connectionId, session);
      if (!projectId) {
        unassigned += 1;
        continue;
      }
      const key = projectSessionKey(projectId, session.id);
    ...
    const merged = partial
      ...
      : next;
    // Re-check after the per-session awaits: a stop during binding resolution
    // must abort the commit before the first mutation or event emission.
    if (!isGenerationCurrent(connectionId, generation)) return false;
  ...
  const fetchConnectionSnapshot = async (connectionId, profile, adapter, generation) => {
    const context = {
      profile,
      canonicalPath: null,
      credentialProvider,
    ...
    await applySnapshot(connectionId, pages, { partial, generation });
  };

  const scheduleSnapshotRefresh = (connectionId) => {
    const state = getConnectionState(connectionId);
    if (state.refreshTimer) return;
    const generation = currentGeneration(connectionId);
    state.refreshTimer = setTimeout(() => {
      state.refreshTimer = null;
      if (!isGenerationCurrent(connectionId, generation)) return;
    ...
    void (async () => {
      while (!abort.signal.aborted) {
        ...
        try {
          ...
          // The stream may resolve after a stop: without this check the
          // continuation would mark the connection live and schedule a
          // refresh that resurrects the deleted state.
          if (!isGenerationCurrent(connectionId, generation)) return;
          if (!response.ok || !response.body) {
            throw new Error(`event stream unavailable (${response.status})`);
          }
          markConnectionLive(connectionId);
          // The stream may (re)connect before or long after the initial
          // snapshot pass — e.g. the local OpenCode runtime was still
          // starting at boot, so the first refresh failed and nothing retried
          // it. Re-baseline on every successful connect (debounced) so a late
          // or reconnecting runtime cannot leave the index permanently stale.
          ...
          for (;;) {
            ...
            if (done) break;
            // A chunk may be delivered after the stop (the abort only settles
            // the next read): drop it instead of reviving the connection.
            if (!isGenerationCurrent(connectionId, generation)) return;
  ...
  const refreshConnection = async (connectionId, options = {}) => {
    ...
    const task = (async () => {
      ...
      if (!isGenerationCurrent(connectionId, generation)) return null;
      if (!resolved) {
      ...
      const release = connectionBroker.acquireLease(connectionId);
      try {
        await fetchConnectionSnapshot(connectionId, profile, adapter, generation);
      } catch (error) {
        if (!isGenerationCurrent(connectionId, generation)) return null;
  ...
  /** Stops the connection's observer, drops its indexed state and emits
   * removal events so subscribed clients drop its sessions immediately.
   * Called when a connection profile is deleted or replaced: without this the
   * observer would keep retrying a disposed adapter forever (holding its
   * lease) and keep emitting freshness events for a connection that no
   * longer exists. */
  const stopObservingConnection = (connectionId) => {
    // Bump the generation BEFORE clearing the observer: any async
    // continuation that lands after this call must see itself as obsolete.
    stoppedGenerations.set(connectionId, currentGeneration(connectionId) + 1);
    clearObserver(connectionId);
  ...
  const ensureObserved = async (connectionId) => {
    ...
    const task = (async () => {
      const resolved = await connectionBroker.resolveConnection(connectionId);
      if (disposed || !resolved || !isGenerationCurrent(connectionId, generation)) return;
      if (getConnectionState(connectionId).observer) return;
