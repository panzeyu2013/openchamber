...
import {
  PROJECT_RUNTIME_UPGRADE_MARKER,
  handleProjectUpgrade,
  isForwardableProjectRuntimePath,
  isProjectRuntimeCapabilityUnavailablePath,
  isAllowedProjectUpgradePath,
  parseProjectRuntimePath,
  registerProjectRuntimeProxyRoutes,
  stripProjectUrlAuthToken,
} from './runtime-proxy.js';

const createCatalogStoreStub = (projects) => ({
  getProject: async (projectId) => projects.find((project) => project.id === projectId) ?? null,
});
...
const routeApp = (dependencies) => {
  ...
  registerProjectRuntimeProxyRoutes(app, dependencies);
  return routes;
};
...
describe('parseProjectRuntimePath', () => {
  it('parses the project id and rest path', () => {
    expect(parseProjectRuntimePath('/api/projects/abc-123/runtime/api/session?x=1')).toEqual({
      projectId: 'abc-123',
  ...
  it('returns null for paths outside the runtime prefix', () => {
    expect(parseProjectRuntimePath('/api/projects')).toBeNull();
    expect(parseProjectRuntimePath('/api/projects/abc-123')).toBeNull();
    expect(parseProjectRuntimePath('/api/fs/list')).toBeNull();
  });

  it('decodes percent-encoded project ids', () => {
    expect(parseProjectRuntimePath('/api/projects/a%20b/runtime/api/x')).toEqual({
      projectId: 'a b',
      restPath: '/api/x',
    });
  });

  it('fails closed on malformed percent-encoding instead of throwing', () => {
    expect(parseProjectRuntimePath('/api/projects/%zz/runtime/api/x')).toBeNull();
    expect(parseProjectRuntimePath('/api/projects/%/runtime/api/x')).toBeNull();
    expect(parseProjectRuntimePath('/api/projects/%e0%a4%a/runtime/api/x')).toBeNull();
  });
});

describe('stripProjectUrlAuthToken', () => {
  it('removes only the control-plane URL token and preserves other query parameters', () => {
    expect(stripProjectUrlAuthToken('/api/session?oc_url_token=control-plane-secret&limit=25'))
      .toBe('/api/session?limit=25');
    expect(stripProjectUrlAuthToken('/api/session?limit=25')).toBe('/api/session?limit=25');
  });
});

describe('project runtime path allowlist', () => {
  it('allows project-capable SDK and RuntimeAPI paths only', () => {
    for (const allowed of [
      ...
      expect(isForwardableProjectRuntimePath(allowed)).toBe(true);
    }
  });
  ...
  it('rejects control-plane namespaces and prefix collisions', () => {
    for (const rejected of [
      ...
      expect(isForwardableProjectRuntimePath(rejected)).toBe(false);
    }
    expect(isProjectRuntimeCapabilityUnavailablePath('/api/config/settings')).toBe(true);
    expect(isProjectRuntimeCapabilityUnavailablePath('/api/configuration')).toBe(false);
  });
});

describe('project runtime proxy', () => {
  it('returns an explicit capability-unavailable response for config CRUD', async () => {
    const adapter = createAdapterStub(async () => {
      throw new Error('config must not cross the project boundary');
    });
    const app = routeApp({
    ...
    const handler = findHandler(app, '*', '/api/projects/:projectId/runtime');
    const response = createResponse();
    await handler(createRequest({
      path: '/api/projects/ws-1/runtime/api/config/settings',
      method: 'PATCH',
      body: { theme: 'dark' },
    }), response);
  ...
  it('forwards an /api path to the project adapter with the rest path', async () => {
    let forwarded;
    ...
    const response = createResponse();
    await handler(createRequest({ path: '/api/projects/ws-1/runtime/api/session' }), response);

    expect(forwarded.context.canonicalPath).toBe('/project/a');
  ...
  it('preserves the query string when forwarding', async () => {
    ...
    const response = createResponse();
    await handler(createRequest({
      path: '/api/projects/ws-1/runtime/api/session?oc_url_token=control-plane-secret&limit=25&directory=%2Fsafe&cursor=abc',
    }), response);
  ...
  it('cancels the upstream stream when the browser disconnects', async () => {
    ...
    const request = createRequest({
      path: '/api/projects/ws-1/runtime/api/global/event',
  ...
  it('pauses on write backpressure and resumes on drain', async () => {
    ...
    const pending = handler(createRequest({ path: '/api/projects/ws-1/runtime/api/session' }), response);
  ...
  it('rejects paths outside /api (no arbitrary proxying)', async () => {
    ...
    const handler = findHandler(app, '*', '/api/projects/:projectId/runtime');
    const response = createResponse();
    await handler(createRequest({ path: '/api/projects/ws-1/runtime/health' }), response);
  ...
  it('404s an unknown project before touching the adapter', async () => {
    let calls = 0;
    const adapter = createAdapterStub(async () => {
      calls += 1;
      return new Response('x', { status: 200 });
    ...
    const handler = findHandler(app, '*', '/api/projects/:projectId/runtime');
    const response = createResponse();
    await handler(createRequest({ path: '/api/projects/ghost/runtime/api/session' }), response);

    expect(calls).toBe(0);
    expect(response.statusCode()).toBe(404);
    expect(JSON.parse(response.body).code).toBe('catalog_project_not_found');
  });
  ...
  it('rejects disallowed methods', async () => {
    ...
    const handler = findHandler(app, '*', '/api/projects/:projectId/runtime');
    const response = createResponse();
    await handler(createRequest({ path: '/api/projects/ws-1/runtime/api/session', method: 'TRACE' }), response);

    expect(calls).toBe(0);
    expect(response.statusCode()).toBe(405);
  });

  it('rejects oversized request bodies before resolving a project', async () => {
    let calls = 0;
    const adapter = createAdapterStub(async () => {
      calls += 1;
    ...
    const handler = findHandler(app, '*', '/api/projects/:projectId/runtime');
    const response = createResponse();
    await handler(createRequest({ path: '/api/projects/ws-1/runtime/api/session', method: 'POST', body: '12345' }), response);
  ...
  it('streams the upstream body and sanitized headers only', async () => {
    ...
    const handler = findHandler(app, '*', '/api/projects/:projectId/runtime');
    const response = createResponse();
    await handler(createRequest({ path: '/api/projects/ws-1/runtime/api/global/event', headers: { accept: 'text/event-stream' } }), response);
  ...
  it('preserves safe typed adapter errors instead of disguising boundary failures as 502', async () => {
    ...
    const handler = findHandler(app, '*', '/api/projects/:projectId/runtime');
    const response = createResponse();
    await handler(createRequest({ path: '/api/projects/ws-1/runtime/api/fs/list?path=%2Fetc' }), response);

    expect(response.statusCode()).toBe(403);
    expect(JSON.parse(response.body)).toEqual({
      error: 'directory is outside the project',
      code: 'catalog_path_outside_project',
    });
  });
});

describe('project WebSocket upgrade dispatcher', () => {
  const project = {
    id: 'ws-1', connectionId: 'local', canonicalPath: '/a', path: '/a', label: 'A', orderKey: '', createdAt: 1, updatedAt: 1,
  };
  ...
  const runUpgrade = (url, deps) => handleProjectUpgrade(createUpgradeRequest(url), {}, null, deps);

  const createUpgradeDeps = (overrides = {}) => {
    const rejections = [];
    const deps = {
      catalogStore: createCatalogStoreStub([project]),
  ...
  it('allowlists only the project-prefixed event and terminal sockets', () => {
    for (const allowed of [
      '/api/event/ws',
      '/api/global/event/ws',
      '/api/terminal/ws',
    ]) {
      expect(isAllowedProjectUpgradePath(allowed)).toBe(true);
    }
    for (const rejected of [
      '/api/session',
      '/api/event',
      ...
      expect(isAllowedProjectUpgradePath(rejected)).toBe(false);
    }
  });

  it('leaves non-project upgrades to the existing module listeners', async () => {
  ...
  it('owns a project-prefixed upgrade once and never twice', async () => {
    const { deps } = createUpgradeDeps();
    const req = createUpgradeRequest('/api/projects/ws-1/runtime/api/event/ws');
    const first = handleProjectUpgrade(req, {}, null, deps);
    expect(req[PROJECT_RUNTIME_UPGRADE_MARKER]).toBe(true);
    expect(await handleProjectUpgrade(req, {}, null, deps)).toBe(false);
    expect(await first).toBe(true);
  });

  it('rejects unauthenticated project upgrades with 401', async () => {
    const { deps, rejections } = createUpgradeDeps({
      getUiAuthController: () => ({ enabled: true, ensureSessionToken: async () => null }),
    });
    expect(await runUpgrade('/api/projects/ws-1/runtime/api/event/ws', deps)).toBe(true);
    expect(rejections).toEqual([{ status: 401, message: expect.stringContaining('authentication') }]);
  });

  it('rejects project upgrades from disallowed origins with 403', async () => {
    const { deps, rejections } = createUpgradeDeps({
  ...
  it('rejects project upgrades for connections without the capability', async () => {
    ...
    expect(await runUpgrade('/api/projects/ws-1/runtime/api/terminal/ws', deps)).toBe(true);
    expect(rejections).toEqual([{ status: 501, message: expect.stringContaining('terminal') }]);
  });

  it('rejects unknown projects and non-socket paths with 404', async () => {
    const { deps: missingDeps, rejections: missingRejections } = createUpgradeDeps({ catalogStore: createCatalogStoreStub([]) });
    expect(await runUpgrade('/api/projects/ghost/runtime/api/event/ws', missingDeps)).toBe(true);
    expect(missingRejections).toEqual([{ status: 404, message: expect.stringContaining('Project') }]);

    const { deps: pathDeps, rejections: pathRejections } = createUpgradeDeps();
    expect(await runUpgrade('/api/projects/ws-1/runtime/api/fs/list', pathDeps)).toBe(true);
  ...
  it('surfaces adapter errors as explicit upgrade rejections', async () => {
    ...
    const { deps, rejections } = createUpgradeDeps({ connectionBroker: createBrokerStub(adapter) });
    expect(await runUpgrade('/api/projects/ws-1/runtime/api/event/ws', deps)).toBe(true);
    expect(rejections).toEqual([{ status: 503, message: 'Project WebSocket upgrade failed' }]);
  });
