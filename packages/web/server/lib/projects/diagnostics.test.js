...
import { desensitizeDiagnostics, registerProjectDiagnosticsRoutes } from './diagnostics.js';
...
const rawDiagnosticsFixture = () => ({
  ...
  capabilities: { projectCatalogV1: true },
});
...
describe('desensitizeDiagnostics', () => {
  it('projects the §19 payload shape with counts and revisions', () => {
    ...
    expect(payload.capabilities).toEqual({ projectCatalogV1: true });
  });
...
describe('GET /api/projects/diagnostics', () => {
  it('serves the desensitized snapshot behind the project routes and never leaks secrets', async () => {
    const registry = createRouteRegistry();
    const getDiagnostics = async () => rawDiagnosticsFixture();
    registerProjectDiagnosticsRoutes(registry.app, { getDiagnostics });

    const response = createMockResponse();
    await registry.getRoute('GET', '/api/projects/diagnostics')({}, response);
  ...
  it('returns 500 with a sanitized message when diagnostics fail', async () => {
    const registry = createRouteRegistry();
    registerProjectDiagnosticsRoutes(registry.app, {
