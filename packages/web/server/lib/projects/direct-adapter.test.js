...
import { createDirectProjectAdapter, createSafeUpstreamValidator } from './direct-adapter.js';
...
describe('direct adapter', () => {
  it('canonicalizes remote paths without touching the control plane filesystem', async () => {
    const adapter = createDirectProjectAdapter({ connectionId: 'conn-1', fetchImpl: async () => new Response(), lookupImpl: createLookup(PUBLIC) });
  ...
  it('forwards to the saved baseUrl with the rest path and injected bearer token', async () => {
    let captured;
    const adapter = createDirectProjectAdapter({
      connectionId: 'conn-1',
  ...
  it('probes a remote project path before reporting it as usable', async () => {
    const calls = [];
    const adapter = createDirectProjectAdapter({
      connectionId: 'conn-1',
      fetchImpl: async (url, init) => {
  ...
  it('rejects directory hints outside the project boundary before forwarding', async () => {
    let calls = 0;
    const adapter = createDirectProjectAdapter({
      connectionId: 'conn-1',
      fetchImpl: async () => {
    ...
    ).rejects.toMatchObject({ code: 'catalog_path_outside_project', status: 403 });
    expect(calls).toBe(0);
  });
  ...
  it('accepts project-relative filesystem and Git paths', async () => {
    let calls = 0;
    const adapter = createDirectProjectAdapter({
      connectionId: 'conn-1',
      fetchImpl: async () => {
  ...
  it('openWebSocket blocks blocked auth headers and rejects out-of-project directory hints', async () => {
    const adapter = createDirectProjectAdapter({
      connectionId: 'conn-1',
      fetchImpl: async () => new Response(),
      lookupImpl: createLookup(PUBLIC),
