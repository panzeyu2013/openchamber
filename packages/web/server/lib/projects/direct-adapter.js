 ...
 * HTTP/HTTPS. The control plane proxies project-scoped requests to the
 ...
 *
 * The cache is a fast path for non-credential-bearing checks (probes). Every
 * actual outbound request passes `fresh: true`: DNS is re-resolved
 * immediately before the connection so a rebinding flip between the cached
 * verdict and the fetch (the classic check-then-use window, previously up to
 * the full TTL) cannot send server-side credentials to a private address.
 */
export const createSafeUpstreamValidator = (dependencies = {}) => {
  ...
  const assertSafeUpstreamUrl = async (baseUrl, options = {}) => {
...
export const createDirectProjectAdapter = (dependencies) => {
  const {
    connectionId,
    ...
    timeoutMs = 30_000,
    maxRedirects = 3,
    // The saved target URL at adapter creation time (server-internal). The
    // profile sync compares it against the private profile so a PATCHed
    // baseUrl replaces the adapter and restarts the observer's event stream
    // instead of leaving it bound to the old server. null means "not
    // profile-synced" (e.g. an injected adapter) and never triggers a swap.
    targetUrl = null,
  } = dependencies;
  ...
  const requestWithRedirects = async (baseUrl, pathnameAndSearch, init, context, redirectCount = 0) => {
    // Fresh resolution immediately before the outbound call: the cached
    // verdict may predate a DNS rebinding flip, and this request carries
    // server-side credentials.
    await assertSafeUpstreamUrl(baseUrl, { fresh: true });
  ...
  const listChildren = async (context, directoryPath) => {
    ...
    if (context?.canonicalPath && !isPathWithinProject(context.canonicalPath, directory)) {
      throw directError('catalog_path_outside_project', 403, 'directory is outside the project');
    }
  ...
  const fetch = async (context, request, restPath) => {
    ...
    if (canonicalPath) {
      ...
      // the project canonical path so the target server always resolves
      // the working directory inside the project.
      ...
      for (const hint of readRequestProjectPathHints(scopedRestPath, request)) {
        if (!isPathWithinProject(canonicalPath, hint)) {
          throw directError('catalog_path_outside_project', 403, 'path is outside the project');
        }
      }
    }
  ...
  /**
   * Resolves the upstream ws(s):// URL for a project-scoped WebSocket
   ...
   * itself is created by the project runtime proxy, which applies the
   * connect timeout.
   */
  const openWebSocket = async (context, request = {}) => {
    ...
    // Fresh resolution right before the ws client dials the same host.
    await assertSafeUpstreamUrl(baseUrl, { fresh: true });
    const pathname = getWsPathname(request.path);
    if (!pathname.startsWith('/api/')) {
      throw directError('catalog_runtime_path_not_allowed', 404, 'Path is not a forwardable project socket');
    }
    const headers = await buildUpstreamHeaders(context, request?.headers);
    const canonicalPath = context?.canonicalPath;
    if (canonicalPath) {
      ...
      // lexical boundary check still blocks `..` traversal and arbitrary
      // directories. Both directory-header conventions are overwritten with
      // the project canonical path, matching `fetch`.
      for (const hint of readRequestDirectoryHints(request)) {
        if (!isPathWithinProject(canonicalPath, hint)) {
          throw directError('catalog_path_outside_project', 403, 'directory is outside the project');
        }
