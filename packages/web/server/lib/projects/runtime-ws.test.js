...
import { PROJECT_RUNTIME_UPGRADE_MARKER, handleProjectUpgrade } from './runtime-proxy.js';
import { createRequestSecurityRuntime } from '../security/request-security.js';

const fsPromises = fs.promises;

/** Boots a real HTTP server with the projects runtime pieces wired exactly
 * like the server entrypoint: the central project upgrade dispatcher is the
 * FIRST `upgrade` listener, and a module-style listener (mimicking the
 * terminal runtime) is registered afterwards to prove non-project upgrades
...
const createHarness = async ({ adapter = null, uiAuthController = null } = {}) => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'project-ws-test-'));
  ...
  broker.registerAdapter(adapter ?? createLocalProjectAdapter({
    fs: fsPromises,
    path,
  ...
  const { descriptor: project } = await catalogStore.createProject({
    connectionId: 'local',
  ...
  server.on('upgrade', (req, socket, head) => {
    if (req[PROJECT_RUNTIME_UPGRADE_MARKER]) return;
    let pathname = '';
...
describe('project runtime WebSocket upgrades (integration)', () => {
  let harness;
  afterEach(async () => {
    if (!harness) return;
  ...
  it('forwards a project-prefixed upgrade to the adapter upstream and pipes both ways', async () => {
    harness = await createHarness();
    const socket = await open(`${harness.wsUrl}/api/projects/${harness.project.id}/runtime/api/event/ws`);
    socket.send('ping');
  ...
  it('injects upstream auth and directory headers, and never forwards browser auth', async () => {
    ...
    const socket = await open(`${harness.wsUrl}/api/projects/${harness.project.id}/runtime/api/terminal/ws?oc_url_token=control-plane-token`);
    await new Promise((resolve) => setTimeout(resolve, 20));
    expect(capturedHeaders['x-openchamber-runtime-auth']).toBe('injected-secret');
    expect(capturedHeaders['x-opencode-directory']).toBe('/project/a');
  ...
  it('rejects unauthenticated project upgrades with 401', async () => {
    ...
    await expectRejected(`${harness.wsUrl}/api/projects/${harness.project.id}/runtime/api/event/ws`);
  ...
  it('rejects project upgrades when the connection lacks the capability', async () => {
    ...
    await expectRejected(`${harness.wsUrl}/api/projects/${harness.project.id}/runtime/api/terminal/ws`);
  ...
  it('does not intercept project-prefixed upgrades that are not sockets', async () => {
    harness = await createHarness();
    ...
    await expectRejected(`${harness.wsUrl}/api/projects/${harness.project.id}/runtime/api/fs/list`);
    expect(harness.upgradeRejections).toEqual([{ status: 404, message: expect.stringContaining('forwardable project socket') }]);
    expect(harness.upstreamConnections).toHaveLength(0);
  });
});
