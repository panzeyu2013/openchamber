...
const createApp = () => {
  ...
  return { registry, calls, adapter, sessionIndex, bindingStore, catalogStore, project };
};

describe('GET /api/project-sessions/snapshot', () => {
  it('returns the session index snapshot', async () => {
    const { registry } = createApp();
    const res = createMockResponse();

    await registry.getRoute('GET', '/api/project-sessions/snapshot')(createMockRequest(), res);
...
describe('POST /api/projects/:projectId/sessions', () => {
  it('creates a session through the connection adapter and records a created-in-project binding', async () => {
    const { registry, calls, project } = createApp();
    const res = createMockResponse();

    await registry.getRoute('POST', '/api/projects/:projectId/sessions')(createMockRequest({
      params: { projectId: project.id },
      body: { prompt: 'hello' },
    }), res);

    expect(res.statusCode).toBe(201);
    expect(res.body).toEqual({ sessionId: 'ses-new', projectId: project.id });
  ...
  it('404s an unknown project', async () => {
    ...
    expect(res.body).toEqual({ error: 'Project not found', code: 'catalog_project_not_found' });
  });

  it('502s when the upstream session creation fails', async () => {
    const { registry, calls, adapter, project } = createApp();
...
describe('POST /api/projects/:projectId/sessions/:sessionId/bind', () => {
  it('binds explicitly with allowMove and a directory override', async () => {
    const { registry, calls, project } = createApp();
    const res = createMockResponse();

    await registry.getRoute('POST', '/api/projects/:projectId/sessions/:sessionId/bind')(createMockRequest({
      params: { projectId: project.id, sessionId: 'ses-1' },
      body: { directory: '/custom/path' },
    }), res);

    expect(res.statusCode).toBe(200);
    expect(res.body).toEqual({ bound: true, projectId: project.id, upstreamSessionId: 'ses-1' });
  ...
  it('defaults the observed directory to the project canonical path', async () => {
    const { registry, calls, project } = createApp();
    const res = createMockResponse();
  ...
  it('404s an unknown project', async () => {
    ...
    await registry.getRoute('POST', '/api/projects/:projectId/sessions/:sessionId/bind')(createMockRequest({
      params: { projectId: 'ghost-project', sessionId: 'ses-1' },
      body: {},
    }), res);
  ...
  it('propagates a binding conflict with its status and code', async () => {
    const { registry, bindingStore, project } = createApp();
    ...
    expect(res.body).toEqual({
      error: 'session binding already points at a different project; retry with an explicit move',
      code: 'binding_conflict',
    });
  });
});

describe('GET /api/project-sessions/events', () => {
  it('streams SSE events to the client and unsubscribes on close', async () => {
    ...
    await registry.getRoute('GET', '/api/project-sessions/events')(req, res);
