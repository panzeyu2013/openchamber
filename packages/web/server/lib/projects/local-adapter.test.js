...
import { createLocalProjectAdapter, LOCAL_CONNECTION_ID, localConnectionCapabilities } from './local-adapter.js';
...
const createAdapter = (overrides = {}) => createLocalProjectAdapter({
  fs: fsPromises,
  path,
  ...overrides,
});
...
describe('createLocalProjectAdapter', () => {
  ...
  describe('listChildren', () => {
    ...
    it('rejects parent traversal out of the project boundary', async () => {
      const projectDir = path.join(tempDir, 'project');
      fs.mkdirSync(projectDir);
      const adapter = createAdapter();
      await expectTypedError(
        adapter.listChildren({ canonicalPath: projectDir }, path.join(projectDir, '..', 'secret')),
        'catalog_path_outside_project',
        403,
      );
    });

    it('rejects a symlink that escapes the project boundary', async () => {
      ...
      const adapter = createAdapter();
      await expectTypedError(
        adapter.listChildren({ canonicalPath: projectDir }, path.join(projectDir, 'escape')),
        'catalog_path_outside_project',
        403,
      );
    });

    it('allows listing the project root and real descendants', async () => {
      const projectDir = path.join(tempDir, 'project');
      const subDir = path.join(projectDir, 'sub');
      fs.mkdirSync(subDir, { recursive: true });
      const adapter = createAdapter();
      expect((await adapter.listChildren({ canonicalPath: projectDir }, projectDir)).directory).toBe(projectDir);
      expect((await adapter.listChildren({ canonicalPath: projectDir }, subDir)).directory).toBe(subDir);
    });
  });
  ...
  describe('openWebSocket', () => {
    const wsProjectDir = () => {
      const dir = path.join(tempDir, 'ws-project');
      fs.mkdirSync(dir, { recursive: true });
      return dir;
    };

    const createWsAdapter = (dependencies = {}) => createLocalProjectAdapter({
      fs: fsPromises,
      path,
    ...
    it('overwrites the directory header with the canonical path', async () => {
      ...
      const spec = await adapter.openWebSocket(
        { canonicalPath: wsProjectDir() },
        { path: '/api/event/ws', headers: { 'x-opencode-directory': path.join(wsProjectDir(), 'sub') } },
      );
      expect(spec.headers['x-opencode-directory']).toBe(wsProjectDir());
    });

    it('rejects directory hints outside the project and non-/api paths', async () => {
      const adapter = createWsAdapter();
      await expectTypedError(
        adapter.openWebSocket({ canonicalPath: wsProjectDir() }, { path: '/api/terminal/ws', headers: { 'x-opencode-directory': '/etc' } }),
        'catalog_path_outside_project',
        403,
      );
      await expectTypedError(
        adapter.openWebSocket({ canonicalPath: wsProjectDir() }, { path: '/health' }),
  ...
  describe('fetch directory boundary enforcement', () => {
    ...
    const createForwardingAdapter = (dependencies = {}) => createLocalProjectAdapter({
      fs: fsPromises,
      path,
      buildOpenCodeUrl: (restPath) => `http://opencode.test${restPath}`,
      getOpenCodeAuthHeaders: async () => ({ 'x-openchamber-runtime-auth': 'secret' }),
    ...
    it('injects the project directory when the request carries no hint', async () => {
      ...
      expect(forwarded.init.headers.get('x-opencode-directory')).toBe(projectDir());
      expect(forwarded.init.headers.get('x-openchamber-runtime-auth')).toBe('secret');
    });

    it('strips the /api prefix like the generic renderer proxy (proxy.js)', async () => {
      const adapter = createForwardingAdapter();
      const forwarded = await adapter.fetch(
        { canonicalPath: projectDir() },
      ...
      expect(forwarded.url).toBe('http://opencode.test/global/event');
      const snapshot = await adapter.fetch(
        { canonicalPath: projectDir() },
        createRequest(),
        '/api/experimental/session?archived=false&limit=50',
      );
      expect(snapshot.url).toBe('http://opencode.test/experimental/session?archived=false&limit=50');
    });
    ...
    it('copies Headers-instance headers in the correct order (value, key)', async () => {
      const adapter = createForwardingAdapter();
      const forwarded = await adapter.fetch(
        { canonicalPath: projectDir() },
        createRequest({ headers: new Headers({ accept: 'text/event-stream', cookie: 'browser-secret' }) }),
        '/api/global/event',
      );
      expect(forwarded.init.headers.get('accept')).toBe('text/event-stream');
      expect(forwarded.init.headers.get('cookie')).toBeNull();
    });

    it('overwrites a client-supplied directory header inside the project', async () => {
      const adapter = createForwardingAdapter();
      const forwarded = await adapter.fetch(
        { canonicalPath: projectDir() },
        createRequest({ headers: { 'x-opencode-directory': path.join(projectDir(), 'sub') } }),
        '/api/session',
      );
      expect(forwarded.init.headers.get('x-opencode-directory')).toBe(projectDir());
    });

    it('rejects a client-supplied directory header outside the project', async () => {
      const adapter = createForwardingAdapter();
      await expectTypedError(
        adapter.fetch(
          { canonicalPath: projectDir() },
      ...
      await expectTypedError(
        adapter.fetch(
          { canonicalPath: projectDir() },
          createRequest({ headers: { 'x-opencode-directory': path.join(projectDir(), '..', 'secret') } }),
          '/api/session',
        ),
        'catalog_path_outside_project',
        403,
    ...
    it('rejects a directory query parameter outside the project', async () => {
      const adapter = createForwardingAdapter();
      await expectTypedError(
        adapter.fetch(
          { canonicalPath: projectDir() },
    ...
    it('scopes filesystem directory listing queries to the project', async () => {
      let capturedUrl = '';
      const adapter = createForwardingAdapter({
      ...
      expect(capturedUrl).toBe(`/fs/list?path=${encodeURIComponent(projectDir())}`);
      await expectTypedError(
        adapter.fetch(
          { canonicalPath: projectDir() },
          createRequest(),
    ...
    it('rejects filesystem and terminal path fields outside the project', async () => {
      const adapter = createForwardingAdapter();
      for (const [restPath, body] of [
        ['/api/fs/mkdir', { path: '/etc/new-dir' }],
        ['/api/fs/write', { path: '/etc/file', content: 'x' }],
        ['/api/fs/delete', { path: '/etc/file' }],
        ['/api/fs/rename', { oldPath: path.join(projectDir(), 'file'), newPath: '/etc/file' }],
    ...
    it('resolves project-relative file and Git paths against the project root', async () => {
      const adapter = createForwardingAdapter();
      const forwarded = await adapter.fetch(
        { canonicalPath: projectDir() },
    ...
    it('preserves the server-validated outside-file grant exception for local reads', async () => {
      ...
      const forwarded = await adapter.fetch(
        { canonicalPath: projectDir() },
        createRequest(),
        '/api/fs/read?path=%2Ftmp%2Feditor-only.txt&allowOutsideProject=true&outsideFileGrant=grant-1',
      );
      expect(forwarded.url).toContain('allowOutsideProject=true');
      expect(forwarded.url).toContain('outsideFileGrant=grant-1');
    });

    it('rejects a directory body field outside the project', async () => {
      const adapter = createForwardingAdapter();
      await expectTypedError(
        adapter.fetch(
          { canonicalPath: projectDir() },
    ...
    it('does not enforce a boundary when no project path is in context', async () => {
      const adapter = createForwardingAdapter();
